:- module(vertical,[vertical/6]).
:- use_module(helpeFunctions).

vertical(Bombs1, Bombs2, Horiz, Rows, Column, NewList):-
transportMatriex(Rows, Column, List),
ignorBombs(Bombs1, Bombs2, List,[Horiz], List2),
...
H2 \= Bombs1,
H2 \= Bombs2,
not(isMember(H1, List)),
not(isMember(H2, List)),
